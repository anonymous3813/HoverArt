<script lang="ts">
	...
	import { auth, setAuth, isLoggedIn } from '$lib/auth.svelte.ts';
...
<main class="flex min-h-screen items-center justify-center px-4" style="font-family: 'Space Mono', monospace;">
	<div class="w-full max-w-sm">
		<div class="mb-8 text-center">
			...
			<p class="mt-2 text-xs tracking-widest text-white/30 uppercase">Welcome back</p>
		</div>
		...
		<p class="mt-5 text-center text-xs text-white/30">
			No account?
			<a href="/register" class="text-[#00f5ff] hover:underline">Create one</a>
		</p>
	</div>
</main>

...
function blockLayout(canvasW: number, canvasH: number, cols: number, rows: number) {
    ...
    let bh = rows > 0 ? (availH - (rows - 1) * gap) / rows : 18 * s;
...
function blockRect(c: number, r: number, canvasW: number, canvasH: number, cols: number, rows: number) {
...
function explodeCluster(cells: BlockCell[], cols: number, rows: number, sc: number, sr: number): number {
    const start = cellIndex(sc, sr, cols);
    if (!cells[start]?.alive)
        return 0;
...
function circleRectResolve(bx: number, by: number, r: number, left: number, top: number, w: number, h: number): {
    hit: boolean;
    nx: number;
    ny: number;
    pen: number;
...
export function updateGame(state: BreakoutState, dt: number, canvasW: number, canvasH: number, facePaddle01: number | null): void {
...
export function drawBreakout(ctx: CanvasRenderingContext2D, state: BreakoutState, canvasW: number, canvasH: number, faceDebug01: number | null) {
    const s = scaleFor(canvasW);
    const { margin, paddleW, paddleH, paddleY, ballR } = paddleMetrics(canvasW, canvasH);
    ...
    const faceHint = faceDebug01 != null ? ` face ${(faceDebug01 * 100).toFixed(0)}%` : ' face —';

...
<<<<<<< HEAD:hover-art-frontend/src/lib/boxing/GameEngine.ts
    ...
    async init() {
        ...
        const [playerAnims, cpuAnims] = await Promise.all([
            loadMixamoAnimations(playerMixer, this.playerRoot, animPaths, (n, t) => this.cb.onProgress?.(20 + Math.round((n / t) * 35), `Animations ${n}/${t}…`)),
            loadMixamoAnimations(cpuMixer, this.cpuRoot, animPaths, (n, t) => this.cb.onProgress?.(55 + Math.round((n / t) * 35), `CPU animations ${n}/${t}…`))
        ]);
        for (const [name, action] of playerAnims)
            this.playerCtrl.addClip(name, action);
        for (const [name, action] of cpuAnims)
            this.cpuCtrl.addClip(name, action);
    ...
    _buildRing() {
        const R = RING_HALF;
        const floor = new THREE.Mesh(new THREE.PlaneGeometry(R * 2, R * 2), new THREE.MeshStandardMaterial({ map: this._makeRingTexture(512), roughness: 0.9 }));
        ...
        const crowd = new THREE.Mesh(new THREE.TorusGeometry(9, 0.8, 8, 64), new THREE.MeshStandardMaterial({
            color: 0x110800,
            emissive: 0xff6600,
    ...
    _prepareCharacter(fbx: THREE.Group, isPlayer: boolean) {
        ...
        this.scene.add(fbx);
        if (isPlayer) {
            const cameraOffset = new THREE.Vector3(-2, 2.5, 6);

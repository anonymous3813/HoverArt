<<<<<<< HEAD:hover-art-frontend/src/lib/components/TwoTruthsLieGame.svelte
...
<script lang="ts">
	...
	type Clip = { faceSamples: FaceSample[]; voiceSamples: VoiceSnapshot[] };
	...
	if (typeof window !== 'undefined') {
		speechSupported = !!(window.SpeechRecognition || (window as unknown as { webkitSpeechRecognition?: unknown }).webkitSpeechRecognition);
	}
	...
	function startPhraseRecord() {
		if (!faceLandmarker || !voice) return;
		clips = clips.map((c, i) =>
			i === phraseIndex ? { faceSamples: [], voiceSamples: [] } : c
		);
		recordStart = performance.now();
		lastTs = -1;
		recordRemaining = RECORD_MAX_SEC;
	...
	function loopRecord() {
		animId = requestAnimationFrame(loopRecord);
		const now = performance.now();
		if (!faceLandmarker || !voice || phase !== 'recording') return;
...
>>>>>>> 68aa9ff59fba30d9b0ec6d395e6fc1f0bb7b10b9:hover-art-frontend/src/routes/games/two-truths/TwoTruthsLieGame.svelte
</script>

...
export async function analyzeTwoTruthsLie({ phrases }) {
    ...
    if (!key) {
        throw new Error('No OpenAI API key found. Put OPENAI_SECRET=sk-... in hover-art-backend/.env, save the file ' +
            '(unsaved editor tabs are not read), then restart the backend with npm start in that folder.');
    }
    const userPayload = {
        instructions: 'The player recorded three short spoken statements. Exactly two are true and one is a lie, but you do not know which. You receive (1) rough face-motion heuristics per clip, (2) rough voice/pause heuristics per clip — these are noisy and NOT biometric truth data. (3) Your job is NLP: plausibility, internal consistency, timeline impossibilities, and whether a claim sounds fabricated or extremely rare.',
        phrases
    };
